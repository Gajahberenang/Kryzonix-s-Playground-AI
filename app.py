...
app = FastAPI(
    title="Kryzonix's Playground",
    description="A private AI workspace for thinking, creating, and getting things done.",
    version="1.0.0",
)
...
async def serve_privacy(request: Request):
    return serve_html_with_nonce(request, abs_join(BASE_DIR, "static/privacy.html"))

@app.get("/terms")
async def serve_terms(request: Request):
    return serve_html_with_nonce(request, abs_join(BASE_DIR, "static/terms.html"))

@app.get("/thank-you")
async def serve_thank_you(request: Request):
    return serve_html_with_nonce(request, abs_join(BASE_DIR, "static/thank-you.html"))
...
async def serve_sitemap():
    return PlainTextResponse(
        ...
        '<url><loc>/</loc></url><url><loc>/login</loc></url>'
        '<url><loc>/privacy</loc></url><url><loc>/terms</loc></url>'
        '</urlset>',
...
async def receive_analytics(request: Request):
    """Accept anonymous, consent-based page views without persisting identity."""
    try:
        data = await request.json()
        path = str(data.get("path", "/"))[:200].replace("\n", " ")
        referrer = str(data.get("referrer", ""))[:200].replace("\n", " ")
        logger.info("analytics pageview path=%s referrer=%s", path, referrer)
    except Exception:
        pass
    return {"ok": True}
...
async def serve_not_found(path: str, request: Request):
    response = serve_html_with_nonce(request, abs_join(BASE_DIR, "static/404.html"))
